...
App settings, environment values, commands, ports, data mounts, and metadata remain based on the installed configuration. Custom Compose apps use the same flow. Updates select and save an explicit numbered release tag; they do not install an unidentified build behind `latest`.

The default check lists registry tags and offers the highest compatible stable release for the installed OS and CPU architecture. It supports two-, three-, and four-part version numbers, an optional `v` prefix, and LinuxServer `-lsN` package revisions. Development, nightly, prerelease, and commit tags are excluded. Known installed release numbers prevent downgrades within a version scheme. Calendar versions and LinuxServer `-lsN` releases are kept separate: for example, `v26.2-ls250` must not be ordered against the historical date tag `2021.11.25`. Explicit packaging variants such as `rootless`, `alpine`, and `slim` are preserved. Short channels such as `16` or `1.25` constrain the initial selection to that major/minor series. Full release tags can receive newer major versions, shown for confirmation before installation. Explicit digest pins remain pinned.

For example, an app configured with `gitea/gitea:latest` can offer `gitea/gitea:1.26.0` as **1.26.0**, even when that image publishes no version label. If no compatible numbered release can be verified, or tag discovery fails, the check shows an explanation and does not fall back to an unversioned image. Installed images without version metadata can still display a **Build** identifier; that is an image ID, not evidence of a development build. Legacy explicit `source=registry` checks remain informational and follow the configured tag.

When version labels are absent, the checker first tries to identify the installed image by matching its immutable ID to a numbered release (within the existing 20-manifest check limit). If it cannot identify the installed release, it blocks the offer because downgrade direction is unknown.

When every service already uses the exact offered image ID, the UI shows **Pin version** under **Version pinning**, without adding to the software-update count. The installed release number is recovered from the matching tag. The confirmation explains that only image references change and the software stays the same; the existing apply flow may briefly stop the app. Any changed service image, including a sidecar or a rebuild with the same version label, remains an **Update**.
...
- `POST /v2/app_management/updates/check`: prepare image updates from the installed configuration without accessing marketplaces. Responses include `update_kind` (`update` or `pin`), `current_version`, `target_version`, `update_ready`, `update_token`, and per-service `registry_images`.
