...
The default check lists registry tags and offers the highest compatible stable release for the installed OS and CPU architecture. It supports two-, three-, and four-part version numbers, an optional `v` prefix, and LinuxServer `-lsN` package revisions. Development, nightly, prerelease, and commit tags are excluded. Known installed release numbers prevent downgrades. Explicit packaging variants such as `rootless`, `alpine`, and `slim` are preserved. Short channels such as `16` or `1.25` constrain the initial selection to that major/minor series. Full release tags can receive newer major versions, shown for confirmation before installation. Explicit digest pins remain pinned.
